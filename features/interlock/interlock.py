...
h.newpin('paused-by-interlock', hal.HAL_BIT, hal.HAL_OUT)
h.newpin('paused-by-interlock.not', hal.HAL_BIT, hal.HAL_OUT)
...
h['paused-by-interlock'] = False
h['paused-by-interlock.not'] = True
h['pause-alert'] = False
h['exception'] = False
...
try:
  while True:
    try:
      h['pause-program'] = False
      h['paused-by-interlock.not'] = not h['paused-by-interlock']
      h['spindle-stop'] = False
      h['exception'] = False
      ...
      if h['closed']:
        ...
        if h['release']:
          ...
          h['feed-inhibit'] = False
          h['release'] = False
        elif h['start-button-pulse'] and h['paused-by-interlock']:
          h['paused-by-interlock'] = False
          h['spindle-inhibit'] = False
          startTime = time.time()
          # Wait a bit so spindle is up to speed before motion resumes
        ...
        if h['program-running']:
          ...
          # We do want to set feed-inhibit and paused-by-interlock True here, because the program could already be paused when the interlock is opened
          h['feed-inhibit'] = True
          h['paused-by-interlock'] = True
